...
class PDQNAgent(Agent):
    ...
    def __init__(self,
                 ...
                 batch_size=64,
                 gamma=0.99,
                 tau_actor=0.001,  # Polyak averaging factor for copying target weights
                 tau_actor_param=0.0001,
        ...
        self.noise = OrnsteinUhlenbeckActionNoise(self.action_parameter_size, random_machine=self.np_random, mu=0., theta=0.15, sigma=0.01) #, theta=0.01, sigma=0.01)
    ...
    def _optimize_td_loss(self):
        ...
        # ---------------------- optimize Q-network ----------------------更新DQN
        with torch.no_grad():
            pred_next_action_parameters = self.actor_param_target.forward(next_states)  #使用DDPG target
            pred_Q_a = self.actor_target(next_states, pred_next_action_parameters)   #使用DQN target
            Qprime = torch.max(pred_Q_a, 1, keepdim=True)[0].squeeze()

            # Compute the TD error
            # rewards = -1 * rewards  # 取 rewards 的相反数
            rewards = torch.clamp(rewards, min=-1)  # 将 rewards 限制在 [-1, 1] 的范围内

            # target = rewards + 1 + (1 - terminals) * self.gamma * Qprime
        ...
        print(loss_Q)
        if torch.isnan(loss_Q).any():
            print("The value is NaN")
        self.actor_optimiser.zero_grad()
        loss_Q.backward()
        if self.clip_grad > 0:
            torch.nn.utils.clip_grad_norm_(self.actor.parameters(), self.clip_grad)
        self.actor_optimiser.step()                                  #使用adam更新***DQN（actor）***的Q网络参数
        ...
        Q = self.actor(states, action_params)   #DDOGactor网络选择动作
        Q_val = Q
        if self.weighted:
        ...
        self.actor_param_optimiser.step()                #更新***DDPG（actor）***网络参数

        soft_update_target_network(self.actor, self.actor_target, self.tau_actor)    #使用之前优化后的actor来优化target
