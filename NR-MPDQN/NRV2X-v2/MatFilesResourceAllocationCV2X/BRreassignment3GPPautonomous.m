...
NbeaconsT = stationManagement.NbeaconsT;
...
for j=1:phyParams.cv2xNumberOfReplicasMax    
    ...
    C = (subframesToNextAlloc(activeIDsCV2X,j) < simParams.T1autonomousModeTTIs | subframesToNextAlloc(activeIDsCV2X,j)>stationManagement.T2autonomousModeTTIs);
    ...
    if j>1
        ...
        E= subframesToNextAlloc(activeIDsCV2X,1)==3 & stationManagement.newDataIndicator(activeIDsCV2X) & sum(stationManagement.pckBuffer(activeIDsCV2X,1,:),3)*(simParams.resourceReEvaluation);
...
hasNewPacketThisTbeacon = (max(timeManagement.timeLastPacket(activeIDsCV2X,:)) > (timeManagement.timeNow-phyParams.TTI-1e-8));
...
scheduledID_ReEval(max(timeManagement.timeLastPacket(scheduledID_ReEval,:),[],2)<0) = []; % re-evaluation initially disabled
...
stationManagement.resReselectionCounterCV2X(activeIDsCV2X) = stationManagement.resReselectionCounterCV2X(activeIDsCV2X)-stationManagement.hasTransmissionThisSlot;
...
keepCheck_MAC = activeIDsCV2X(stationManagement.resReselectionCounterCV2X(activeIDsCV2X)==0);
% updateCounter_MAC =[];
...
% RC=0,队列中有包就行
scheduledID_MAC = activeIDsCV2X(sum(stationManagement.pckBuffer(activeIDsCV2X,1,:),3)>0 & (stationManagement.resReselectionCounterCV2X(activeIDsCV2X)<=0));
...
needReselectionCounterRestart = [scheduledID_PHY;scheduledID_MAC];
% stationManagement.resReselectionCounterCV2X(needReselectionCounterRestart) = 0.5./stationManagement.RRI(keepCheck_MAC) + round(rand(length(keepCheck_MAC),1,1)./stationManagement.RRI(keepCheck_MAC));
...
if ~isempty(needReselectionCounterRestart)
    ...
    stationManagement.T2autonomousModeTTIs(needReselectionCounterRestart) = stationManagement.RRI(needReselectionCounterRestart)*1000;
    stationManagement.averageTbeacon(needReselectionCounterRestart) = stationManagement.RRI(needReselectionCounterRestart);
    stationManagement.NbeaconsT(needReselectionCounterRestart) = floor(stationManagement.averageTbeacon(needReselectionCounterRestart)./phyParams.Tsf);
    stationManagement.Nbeacons(needReselectionCounterRestart) = appParams.NbeaconsF.*stationManagement.NbeaconsT(needReselectionCounterRestart);
    stationManagement.resReselectionCounterCV2X(needReselectionCounterRestart) = 0.5./stationManagement.RRI(needReselectionCounterRestart) + round(rand(length(needReselectionCounterRestart),1,1)./stationManagement.RRI(needReselectionCounterRestart));
    stationManagement.ReCounterLTE0(needReselectionCounterRestart) = stationManagement.resReselectionCounterCV2X(needReselectionCounterRestart);
end

if simParams.FDalgorithm==1 %adaptive pKeep    x
...
for indexSensingV = 1:Nscheduled
    ...
    T2autonomousModeTTIs = stationManagement.T2autonomousModeTTIs(inda);
%     averageTbeacon = appParams.averageTbeacon(inda);
    ...
    BRidOriginal=stationManagement.BRid(inda,:);
    currentT = mod(floor(timeManagement.timeNow-1),NbeaconsT)+1; 
    ...
    if simParams.averageSensingActive==true % 4G procedure
        ...
        sensingMatrixScheduled = reshape(stationManagement.sensingMatrixCV2X(:,:,inda),[],Nbeacons);
        ...
        sensingMatrixScheduled = reshape(stationManagement.sensingMatrixCV2X(:,:,inda),Nbeacons,[])';%此处不需要每个窗口大小对应位置，因此重组方式不同
        sensingMatrixScheduled = sensingMatrixScheduled(1,:);
    end 
    ...
    for i=1:length(a)
        if timeStartingT>timeManagement.timeLastPacket(inda,a(i))
            timeStartingT=timeManagement.timeLastPacket(inda,a(i))/1000;
        end
    end
    % startingT = mod(floor((timeStartingT/1000+phyParams.TsfGap+1e-7)/phyParams.Tsf),NbeaconsT)+1;
    ...
    if (startingT+T2autonomousModeTTIs+1)<=NbeaconsT
        sensingMatrixScheduled([1:((startingT+simParams.T1autonomousModeTTIs-1)*NbeaconsF),((startingT+T2autonomousModeTTIs)*NbeaconsF+1):Nbeacons]) = inf;
    % IF Both are beyond this beacon period
    elseif (startingT+simParams.T1autonomousModeTTIs-1)>NbeaconsT
        sensingMatrixScheduled([1:((startingT+simParams.T1autonomousModeTTIs-1-NbeaconsT)*NbeaconsF),((startingT+T2autonomousModeTTIs-NbeaconsT)*NbeaconsF+1):Nbeacons]) = inf;
    % IF T1 within, T2 beyond
    else
        sensingMatrixScheduled(((startingT+T2autonomousModeTTIs-NbeaconsT)*NbeaconsF+1):((startingT+simParams.T1autonomousModeTTIs-1)*NbeaconsF)) = inf;
    end 
    ...
    MBest = ceil(NbeaconsT * simParams.ratioSelectedAutonomousMode);     % minimum number of resources that must be in the selection set
    if MBest<=0
...
%     printDebugReallocation(timeManagement.timeNow,scheduledID(indexSensingV),positionManagement.XvehicleReal(stationManagement.activeIDs==scheduledID(indexSensingV)),'reall',BR,outParams);
    ...
    if phyParams.cv2xNumberOfReplicasMax>1
        ...
        bestBR(bestSubframe>currentT+T2autonomousModeTTIs) = -1;
