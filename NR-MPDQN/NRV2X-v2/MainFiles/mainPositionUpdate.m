...
if simParams.typeOfScenario ~= constants.SCENARIO_TRACE % Not traffic trace
    % Call function to update vehicles positions %更新车辆位置
...
if sum(stationManagement.vehicleState(stationManagement.activeIDs)==100)>0
    ...
    if simParams.BRAlgorithm==constants.REASSIGN_BR_STD_MODE_4 && timeManagement.timeNow > phyParams.TTI
        ...
        if ~isempty(indexNewVehicles)
            ...
            % [stationManagement.BRid(stationManagement.activeIDs(indexNewVehicles),1),~] = BRreassignmentRandom(simParams.T1autonomousModeTTIs,stationManagement.T2autonomousModeTTIs,stationManagement.activeIDs(indexNewVehicles),simParams,timeManagement,sinrManagement,stationManagement,phyParams,appParams);
            % stationManagement.NbeaconsT
            ...
            stationManagement.resReselectionCounterCV2X(indexNewVehicles) = 0.5./stationManagement.B(indexNewVehicles)' + round(rand(length(indexNewVehicles),1,1)./stationManagement.B(indexNewVehicles)');
            ...
            stationManagement.lt(indexNewVehicles,3) = round(100*rand(length(indexNewVehicles),1)/10);
            a= stationManagement.lt(indexNewVehicles,3);
            Nin = length(a);
            b = round(100*rand(Nin,1));
            for i = 1:Nin
                Nindex = indexNewVehicles(i);
                stationManagement.pckBuffer(Nindex,1:a(i),3) = b(i) + flip(100*((1:a(i))-1));
                stationManagement.PHIt(Nindex,:) = repmat(stationManagement.pckBuffer(Nindex, 1, 3), 1, length(stationManagement.PHIt));%/*接受端信息年龄*/
                stationManagement.PHIt(Nindex,:) = stationManagement.PHIt(Nindex,:) + round(100*rand(1,length(stationManagement.PHIt)));
            end 
...
if appParams.variabilityGenerationInterval == constants.PACKET_GENERATION_ETSI_CAM
    ...
    timeManagement.generationIntervalDeterministicPart(stationManagement.activeIDs(indexNewVehicles)) = appParams.generationInterval(indexNewVehicles) - appParams.variabilityGenerationInterval/2 + appParams.variabilityGenerationInterval*rand(length(indexNewVehicles),1)';
