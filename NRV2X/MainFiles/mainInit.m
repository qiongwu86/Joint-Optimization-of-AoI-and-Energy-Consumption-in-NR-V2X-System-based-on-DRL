...
if appParams.variabilityGenerationInterval == constants.PACKET_GENERATION_ETSI_CAM
    ...
    appParams.generationInterval(stationManagement.activeIDsCV2X) = 100;
    timeManagement.generationIntervalDeterministicPart(stationManagement.activeIDsCV2X) = 100;
end
...
if appParams.nRSUs>0 && ~strcmpi(appParams.RSU_pckTypeString,'CAM')
    timeManagement.generationIntervalDeterministicPart(end-appParams.nRSUs+1:end) = 0.05*1000;
end
%timeManagement.generationIntervalDeterministicPart(stationManagement.activeIDsCV2X) = appParams.averageTbeacon;
timeManagement.timeNextPacket = Inf * ones(simValues.maxID,4); %(simValues.maxID,4)
% timeManagement.timeNextPacket(stationManagement.activeIDs,3) = round(timeManagement.generationIntervalDeterministicPart(stationManagement.activeIDs) .* rand(length(stationManagement.activeIDs),1), 10);
timeManagement.timeNextPacket(stationManagement.activeIDs,3) = 1+fix(timeManagement.generationIntervalDeterministicPart(stationManagement.activeIDs)' .* rand(length(stationManagement.activeIDs),1));
...
if ismember(constants.V_STATE_LTE_TXRX, stationManagement.vehicleState(stationManagement.activeIDs))
   ...
   if ismember(simParams.BRAlgorithm, [constants.REASSIGN_BR_REUSE_DIS_SCHEDULED_VEH,...
    ...
    if simParams.BRAlgorithm == constants.REASSIGN_BR_STD_MODE_4
        ...
        stationManagement.RRI=(stationManagement.A(ceil(3*rand(length(stationManagement.activeIDsCV2X),1))))';
        ...
        stationManagement.resReselectionCounterCV2X(stationManagement.activeIDs) = 0.5./stationManagement.RRI + round(rand(length(stationManagement.activeIDs),1,1)./stationManagement.RRI);

        stationManagement.ReCounterLTE0 = stationManagement.resReselectionCounterCV2X;
        ...
        stationManagement.sensingMatrixCV2X = zeros(1000,appParams.NbeaconsF,simValues.maxID);%设置为最大的矩阵
        ...
        stationManagement.knownUsedMatrixCV2X = zeros(1000*appParams.NbeaconsF,simValues.maxID);
        ...
        for j=1:phyParams.cv2xNumberOfReplicasMax
            ...
            % [stationManagement.BRid(stationManagement.activeIDs,j),~] = BRreassignmentRandom(simParams.T1autonomousModeTTIs,stationManagement.T2autonomousModeTTIs,stationManagement.activeIDs,simParams,timeManagement,sinrManagement,stationManagement,phyParams,appParams);
        ...
        TTIGen = ceil(timeManagement.timeNextPacket(:,3)/1000/phyParams.TTI);
