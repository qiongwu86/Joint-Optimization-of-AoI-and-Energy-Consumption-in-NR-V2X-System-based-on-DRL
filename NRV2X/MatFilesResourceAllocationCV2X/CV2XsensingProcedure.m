...
stationManagement.sensingMatrixCV2X(:,appParams.NbeaconsF,:) = circshift(stationManagement.sensingMatrixCV2X(:,appParams.NbeaconsF,:),1);
...
% if find(isinf(sensedPowerCurrentTTI_MHz))
%     disp('数组 A 中所有元素都是无穷大。');
% end 
...
stationManagement.sensingMatrixCV2X(1,:,stationManagement.activeIDsCV2X) = sensedPowerCurrentTTI_MHz;   
...
BRids_currentSF = fix(BRids_currentSF);
