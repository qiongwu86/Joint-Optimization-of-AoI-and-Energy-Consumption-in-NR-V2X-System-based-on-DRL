...
if simParams.typeOfScenario == constants.SCENARIO_URBAN    % ETSI-Urban 
    ...
    % end
    Xvehicle = Xvehicle + real(direction).*(v*updateInterval);
    % Return indices
    indexOldVehicles = IDvehicle;
    indexOldVehiclesToOld = IDvehicle;

    IDvehicleExit = find(Xvehicle>Xmax | Xvehicle<0 );

    Xvehicle(IDvehicleExit)=0;
    if IDvehicleExit>0
        indexNewVehicles = IDvehicleExit;
        Xvehicle(indexNewVehicles) = mod(Xvehicle(indexNewVehicles) + real(direction(indexNewVehicles)).*(v(indexNewVehicles)*updateInterval),Xmax);
    else
        indexNewVehicles = [];
    end
end
